...
__Part1__:
**소켓은 네트워크 상에서 동작하는 두 프로그램(프로세스) 간의 양방향 통신을 위한 엔드포인트(종착점)를 제공하는 API** 이다. 양 끝점(서버와 클라이언트)에 소켓이 만들어져 있더라도 데이터를 전송할 수 없다.
데이터를 전송하기 위해서는 **파이프**를 만들어서 소켓을 연결해야 한다. 먼저 서버측에서 소켓을만들고, 소켓에 클라이언트가 파이프를 연결하기를 기다린다. 그리고 데이터는 파이프를 통해 전송된다.
IP 주소와 포트 번호를 사용하여 데이터를 송수신하며, 클라이언트-서버 모델에서 주로 사용된다.
...
<mark><em><strong>소켓은 OS가 제공하는 네트워크 통신을 위한 추상화된 인터페이스이다.</strong></em></mark>

Application 은 TCP 헤더를 직접 만들지 않고, IP routing을 직접 처리하지 않고 그냥 send() / recv() 만 호출한다.
이것이 바로 추상화이다.
...
이 소켓 인터페이스는 어디서 시작되었을까. 1983년 ***BSD 4.2(Berkeley Software Distribution)*** 에서 처음 도입되었다. DARPA 자금 지원 하에 UC Berkeley 연구팀이 TCP/IP 프로토콜을 유닉스 운영체제에 통합하면서 만들어진 API가 바로 ***Berkeley Socket(BSD Socket)*** 이다.

Berkeley Socket의 핵심 설계 철학은 단순하다. ***"네트워크 통신을 기존 유닉스 파일 I/O 모델에 자연스럽게 녹여내자."*** 유닉스에서는 파일을 open()하면 FD를 반환하고, 이후 read()/write()/close()로 조작한다. Berkeley Socket은 이 패턴을 그대로 차용하여 socket()이 반환하는 FD가 기존 open()이 반환하는 FD와 ***동일한 네임스페이스***를 공유하도록 설계했다. 덕분에 프로그래머는 파일을 다루듯 네트워크를 다룰 수 있게 되었다.

이 설계가 40년이 지난 지금까지도 사실상의 표준(de facto standard)으로 살아남은 이유는, 유닉스의 ***"Everything is a file"*** 철학과 완벽하게 맞물렸기 때문이다.

## Kernel Internal: struct socket and struct sock

리눅스/유닉스에서는 소켓도 파일의 일종으로 취급된다. 커널 내부에서 소켓이 어떤 자료구조로 표현되는지 살펴보자.

```
struct file (VFS 계층)
  └─ struct socket (소켓 계층, BSD socket 추상화)
       ├─ struct sock (네트워크 프로토콜 계층, L4)
       │    ├─ struct inet_sock (IPv4 확장)
       │    │    └─ struct tcp_sock / struct udp_sock
       │    ├─ sk_receive_queue (수신 버퍼)
       │    ├─ sk_write_queue (송신 버퍼)
       │    └─ sk_backlog (백로그 큐)
       └─ const struct proto_ops *ops (프로토콜별 연산 함수 포인터 테이블)
```

- ***struct file***: VFS(Virtual File System) 계층의 파일 객체이다. 모든 파일 I/O 연산의 진입점이 된다.
- ***struct socket***: BSD 소켓 추상화 계층이다. 프로토콜에 독립적인 소켓의 공통 속성(상태, 타입, 플래그 등)을 담고 있다.
- ***struct sock***: 실제 네트워크 프로토콜 처리를 담당하는 핵심 구조체이다. TCP라면 struct tcp_sock으로 확장되어 시퀀스 번호, 윈도우 크기, 혼잡 제어 상태 등 TCP 고유의 정보를 관리한다.

socket() 시스템 콜이 호출되면 커널 내부에서는 다음과 같은 흐름이 진행된다.

```
sys_socket()
  → sock_create()      // struct socket 할당
    → inet_create()    // struct sock(inet_sock, tcp_sock 등) 할당 및 프로토콜 초기화
  → sock_map_fd()      // struct file 생성 → FD 테이블에 등록 → FD(정수) 반환
```

## VFS and Socket

커널은 소켓에 대해 ***socket_file_ops*** 라는 전용 file_operations 구조체를 등록한다. 애플리케이션이 read()를 호출하면 VFS 계층을 거쳐 다음과 같이 디스패치된다.

```
read(fd, buf, len)
  → VFS: struct file → f_op->read_iter()
    → socket_file_ops.read_iter → sock_read_iter()
      → sock_recvmsg()
        → 프로토콜별 recvmsg (tcp_recvmsg / udp_recvmsg)
```

이 구조 덕분에 select(), poll(), ***[epoll()](https://klarciel.net/wiki/reactive/reactive-eventloop/#multiplexing)*** 같은 I/O 멀티플렉싱 시스템 콜이 소켓 FD에도 동일하게 동작한다. VFS 추상화가 파일과 소켓의 경계를 지워버렸기 때문이다.
...
|SOCK_RAW|	IP (Raw)	|L4 헤더를 직접 구성, IP_HDRINCL 옵션으로 IP 헤더도 직접 조작 가능|	네트워크 진단 도구, 방화벽, 커스텀 프로토콜|

SOCK_STREAM 소켓이 생성될 때, 커널은 해당 소켓의 Receive Buffer 와 Send Buffer 메모리 공간을 할당한다. 수신 측은 이 수신 버퍼의 가용 공간을 ***TCP Window Size*** 로 상대방에게 광고(advertise)한다. 애플리케이션 처리 속도가 느려 이 버퍼가 가득 차면, 통신은 멈춘다(Flow Control).
...
### Server Socket: SYN Queue and Accept Queue

서버 소켓이 listen() 상태에 들어가면 커널은 두 개의 큐를 생성한다.

- ***SYN Queue (Half-open Queue)***: 클라이언트로부터 SYN을 받았지만 아직 3-way handshake가 완료되지 않은 연결들이 대기하는 큐이다.
- ***Accept Queue (Completed Connection Queue)***: 3-way handshake가 완료되었지만 아직 애플리케이션이 accept()를 호출하지 않은 연결들이 대기하는 큐이다.
...
Client            Server (LISTEN)
  ...
  │←── SYN+ACK ──── │
  │                   │
  │──── ACK ────────→ │  → SYN Queue에서 제거 → Accept Queue에 추가
  │                   │
  │              accept() 호출 → Accept Queue에서 꺼냄 → 새 FD 반환
```

고성능 서버 튜닝에서 이 두 큐의 크기를 제어하는 커널 파라미터가 중요하다.
- `net.core.somaxconn`: Accept Queue의 최대 크기를 결정한다.
- `net.ipv4.tcp_max_syn_backlog`: SYN Queue의 최대 크기를 결정한다.

Accept Queue가 가득 차면 커널은 새로운 연결의 ACK를 무시(drop)한다. 클라이언트는 연결이 된 줄 알지만 서버는 모르는 상황이 발생하며, 결국 타임아웃으로 이어진다. 대규모 트래픽 환경에서 간헐적 연결 실패가 발생한다면, 이 큐 크기를 의심해봐야 한다.

### File Descriptor and I/O Multiplexing
...
***프로토콜(Protocol)*** 이란 서로 다른 시스템 간에 데이터를 교환하기 위해 미리 합의된 규칙의 집합이다. 세상에는 수많은 종류의 컴퓨터와 운영체제가 있다. 이들은 서로 데이터를 처리하는 방식이 다르다. 이러한 이기종 장비 간 통신을 하기 위해서는 **표준 규격**이 있어야 한다.

프로토콜은 세 가지 요소로 구성된다.

| 요소 | 설명 | 예시 |
|------|------|------|
| ***Syntax (구문)*** | 데이터의 형식과 구조 | TCP 헤더의 필드 배치, JSON의 키-값 쌍 형식 |
| ***Semantics (의미)*** | 각 필드와 비트가 나타내는 의미와 그에 따른 동작 | SYN 플래그 = "연결 요청", ACK 플래그 = "수신 확인" |
| ***Timing (순서)*** | 메시지 교환의 순서와 타이밍 | 반드시 SYN → SYN+ACK → ACK 순서로 핸드셰이크 |

## Layering

프로토콜은 왜 계층(Layer)으로 나뉘어 있을까. 네트워크 통신이라는 거대한 문제를 하나의 프로토콜로 해결하려 하면 복잡도가 폭발한다. 계층화는 이 복잡도를 관리하기 위한 핵심 전략이다.

- ***Separation of Concerns (관심사의 분리)***: 각 계층은 자기 역할만 책임진다. TCP는 "데이터가 빠짐없이 순서대로 도착하는가"만 신경 쓰고, IP는 "목적지까지 어떤 경로로 보낼 것인가"만 신경 쓴다.
- ***Replaceability (교체 가능성)***: 하위 계층을 교체해도 상위 계층은 영향을 받지 않는다. Wi-Fi에서 LTE로 물리 매체가 바뀌어도 TCP/IP 프로토콜 스택의 구현 코드는 한 줄도 바꿀 필요 없다.
- ***Complexity Management (복잡도 관리)***: 만약 N개의 애플리케이션과 M개의 네트워크 기술이 있을 때, 계층화 없이 각각을 직접 연결하면 N×M 개의 구현이 필요하다. 계층화하면 N+M 으로 줄어든다.
- ***Independent Evolution (독립적 진화)***: HTTP/1.1 → HTTP/2 → HTTP/3 로 애플리케이션 프로토콜이 진화하는 동안, 이더넷 드라이버는 변경할 필요가 없었다.

이러한 표준 규격 역할을 하는 것이 TCP/IP 프로토콜이며 하드웨어의 차이를 TCP/IP가 중간에 추상화되어 가려준다.
...
계층화의 구체적인 구현 방식이 ***캡슐화(Encapsulation)*** 이다. 데이터가 송신 측에서 네트워크 스택을 타고 내려갈 때, 각 계층은 자기 역할에 필요한 제어 정보(헤더)를 앞에 붙인다.
...
수신 측에서는 이 과정이 정확히 역순으로 진행된다. 각 계층은 자기 헤더만 읽고 벗겨낸 뒤, 상위 계층에 전달한다. 이를 ***역캡슐화(Decapsulation)*** 라 한다. 각 계층이 자기 헤더만 해석하고 나머지는 불투명한 페이로드(Opaque Payload)로 취급하기 때문에, 계층 간 독립성이 보장된다.

# Data Flow: Application to Hardware

send() 호출 한 번으로 애플리케이션 데이터가 상대방에게 도착하기까지, 커널 내부에서는 여러 계층을 거치는 정교한 과정이 진행된다. 이 흐름을 이해하면 네트워크 성능 문제의 원인을 각 계층에서 정확히 짚어낼 수 있다.

## Transmit Path (Send)

### Application Layer

애플리케이션이 send(fd, data, len) 또는 write(fd, data, len)를 호출하면, ***User Mode에서 Kernel Mode로 전환(syscall)*** 이 일어난다. 커널은 FD 테이블에서 해당 FD에 연결된 struct socket을 찾고, proto_ops->sendmsg()를 호출하여 프로토콜 스택으로 데이터를 내려보낸다.

### Socket Layer: sk_buff

커널은 이 시점에 ***sk_buff(socket buffer)*** 를 할당한다. sk_buff는 리눅스 네트워킹 스택의 ***핵심 데이터 구조***이다.

sk_buff의 설계에서 주목할 점은, 할당 시 각 계층의 헤더 공간을 ***미리 예약(headroom)*** 해둔다는 것이다. 이후 각 계층에서 헤더를 추가할 때 skb_push()로 ***포인터만 이동***하면 되므로, 메모리 재할당이나 복사가 발생하지 않는다. 이 설계가 리눅스 네트워크 스택의 고성능을 뒷받침하는 핵심이다.

### Transport Layer (TCP)

TCP 계층에서 수행하는 핵심 작업은 다음과 같다.

- ***MSS(Maximum Segment Size) 단위 분할***: 애플리케이션 데이터를 MSS 크기로 쪼갠다. MSS = MTU(1500) - IP 헤더(20) - TCP 헤더(20) = ***1460 bytes*** 이다 (옵션이 없는 기본값 기준).
- ***TCP 헤더 추가***: Source/Dest Port, Sequence Number, ACK Number, Flags, Window Size, Checksum 등이 포함된다.
- ***Send Buffer 적재***: sk_write_queue에 세그먼트를 넣고, cwnd(Congestion Window)와 rwnd(Receiver Window) 중 작은 값만큼만 전송을 허용한다.
- ***Nagle's Algorithm***: 작은 패킷을 모아서 한 번에 보내 네트워크 효율을 높인다. 실시간 통신에서는 TCP_NODELAY 옵션으로 비활성화한다.

### Network Layer (IP)

IP 계층에서는 다음 작업이 수행된다.

- ***IP 헤더 추가***: Source/Dest IP, TTL(Time To Live), Protocol 필드(6=TCP, 17=UDP), Header Checksum
- ***라우팅 결정***: 커널의 라우팅 테이블(FIB, Forwarding Information Base)을 ***Longest Prefix Match*** 알고리즘으로 조회하여, next-hop IP와 출력 네트워크 인터페이스를 결정한다.

### Data Link Layer

- ***ARP***: next-hop IP 주소를 MAC 주소로 변환한다. 먼저 ARP 캐시를 확인하고, 없으면 ARP Request를 브로드캐스트한다.
- ***이더넷 프레임 구성***: Dest MAC + Source MAC + EtherType(0x0800 = IPv4) + Payload + FCS(Frame Check Sequence, CRC-32)

### Physical Layer

- ***DMA(Direct Memory Access)***: NIC의 DMA 엔진이 메인 메모리에 있는 sk_buff 데이터를 ***직접 읽어간다***. CPU는 TX Ring Buffer에 디스크립터(포인터와 길이 정보)만 기록하면 된다. CPU가 데이터를 한 바이트씩 NIC에 복사하는 것이 아니라, NIC가 스스로 메모리에서 가져가므로 CPU 부하가 최소화된다.
- ***NIC 처리***: Preamble + SFD(Start Frame Delimiter) 추가 → 라인 코딩(Manchester/PAM-4/NRZ) → 전기/광 신호로 변환하여 전송
- ***전송 완료 알림***: 전송 완료 후 ***하드웨어 인터럽트***로 커널에 알리고, 커널은 해당 sk_buff를 해제한다. 고성능 환경에서는 ***NAPI(New API)*** 폴링 방식으로 인터럽트 오버헤드를 줄인다.

## Receive Path

수신 측에서는 이 과정이 역방향으로 진행된다.

```
1. Physical/NIC: 전기/광 신호 수신 → 비트 복원 → FCS 검증 → RX Ring Buffer에 DMA 적재 → 인터럽트/NAPI
        ↓
2. Data Link: MAC 주소 확인(내 것인가?) → 이더넷 헤더 제거 → EtherType으로 상위 프로토콜 핸들러 호출
        ↓
3. Network (IP): IP 헤더 검증(Checksum, TTL) → 라우팅 결정(로컬인가?) → IP 헤더 제거 → Protocol 필드로 TCP/UDP 핸들러 호출
        ↓
4. Transport (TCP): {Source IP, Source Port, Dest IP, Dest Port} 4-tuple로 소켓 찾기 → Sequence Number 확인 → 수신 버퍼(sk_receive_queue) 적재 → ACK 생성/전송
        ↓
5. Application: recv()/read() 호출 시 커널이 수신 버퍼에서 사용자 공간(User Space)으로 데이터 복사
```
...
***[TCP](https://en.wikipedia.org/wiki/Transmission_Control_Protocol)*** 는 순서 보장 및 재전송 매커니즘을 통한 데이터 전송 보장을 지원한다. 이러한 특징을 ***신뢰성 보장 매커니즘(reliability mechanisms)*** 이라 한다.
...
<mark><em><strong>TCP 의 신뢰성은 송신자가 데이터 손실을 감지하고 재전송함으로써 확보된다.</strong></em></mark> TCP 는 데이터 손실을 식별하기 위해 ***재전송 시간 초과(Retransmission timeout, RTO)*** 와 ***중복 누적 확인 응답(DupAcks)*** 이라는 두 가지 기술을 사용한다.
...
TCP Flow Control과 Congestion Control은 모든 Connection의 처리량을 조절하며, 실제 전송 가능량은 ***min(cwnd, rwnd)*** 로 결정된다. 즉, 혼잡 윈도우(cwnd)와 수신 윈도우(rwnd) 중 더 작은 값이 병목이 된다.
...
* **구조:** 핸드셰이크(Handshake)가 끝나면 HTTP 규칙은 사라지고, TCP 위에 아주 얇은 자체 프레임(2~14 byte 헤더)만 씌워서 데이터를 주고받는다. 메시지 기반(Message-oriented)이다.
...
- Sec-WebSocket-Key: 클라이언트가 생성한 16바이트 랜덤 값의 Base64 인코딩 문자열. 서버는 이 값에 RFC 6455에 정의된 매직 GUID(`258EAFA5-E914-47DA-95CA-C5AB0DC85B11`)를 연결(concatenate)한 후 SHA-1 해시 → Base64 인코딩하여 Sec-WebSocket-Accept로 반환한다.
...
- 2단계: 명령 전송 (Direct Routing)
  ...
  - Server-2는 Redis 에서 Car-A의 위치를 조회 (Presence Check)
