...
동시성 자료구조 중에 가장 중요한 자료구조를 뽑아보라고 한다면 ___Queue___ 라고 할 수 다. 큐를 활용하면 쓰레드 간의 소통을 할 수 있다. 그중에서도 ___채널(Channel)___ 은 큐에 더 이상 요소를 추가할 수 없도록 닫는 기능과 채널의 메시지를 대기하는 기능을 추가한 것이다. 채널을 활용하면 비동기적으로 처리되는 스트림을 구현할 수 있다.
