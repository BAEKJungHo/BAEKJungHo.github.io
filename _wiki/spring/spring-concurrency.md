...
동시성 이슈란 코드 측면에서는 여러 스레드가 동시에 __같은 인스턴스의 필드 값을 변경__ 하면서 발생하는 문제를 의미하며, 데이터베이스 측면에서는 __스레드간 공유되는 컬럼의 값을 변경__ 하면서 발생하는 문제를 의미합니다. __핵심은 멀티 스레드 환경에서 `공유 자원(shared resources)`을 변경__ 함으로써 발생되는 문제를 동시성 이슈라고 합니다.
